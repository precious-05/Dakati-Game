#Increase Round results font size to adjust
#Game improvement: Result should be shown after every phase 
#When angel i died then thieves phase is meaningless because there is no angel to save the target
#In Round results such as Round#1 result only the citizen kiled in the voting phase is shown in player card not the player dead in thieves phase
#Only numbers should also not accpeted in player name
...
<RoleRevealScreen>:
    ...
    BoxLayout:
        ...
        Label:
            text: root.role_text  # Use the combined property
            font_size: '42sp'
            bold: True
            color: (0.8, 0.5, 0.9, 1)
            outline_width: 2
...
class PlayerCard(ButtonBehavior, BoxLayout):
    ...
    def on_card_pressed(self, *args):
        # This will now properly handle the click
        if hasattr(self, 'callback'):
            self.callback(self.player_name)

class RoleRevealScreen(ModalView):
    role_text = StringProperty('')  # New property for combined text
    player_name = StringProperty('')
    ...
    def __init__(self, **kwargs):
        ...
        self.size_hint = (0.9, 0.9)  # Makes the modal view 90% of window size
...
class PrinciplesScreen(ModalView):
    ...
    "3. The Angel does not know the identity of anyone, not the thieves or the normal Citizens",
    "4. In the first phase {Voting Phase} all players vote to eliminate a suspicious player but thieves cannot vote against each other",
    "5. The player who receives the most votes is eliminated, no matter their role {Thief, Angel or Citizen}",
    "6. In the second phase {Thieves’ Phase} the two Thieves secretly select one alive player to attack. They can even target the Angel by mistake",
    "7. In the third phase {Angel’s Phase} the Angel guesses who the Thieves targeted and tries to save that person {can save themselves too}",
    "8. If the Angel guesses correctly, the target is saved. If the guess is wrong, the target is eliminated.",
    "9. The game continues with these phases until either the Thieves are all eliminated {Citizens win} or the Thieves overpower the Citizens (Thieves win)"
                ]
...
class DakatiGame(BoxLayout):
    ...
    def setup_background(self, phase=None):
        """Set appropriate background based on game phase"""
        if phase is None:
    ...
    # Animated logo text
        ...
        self.slideshow = Image(
            source='assets/slide1.jpg',
            size_hint=(1.4,1.2),
              # Good dimensions for 1000×750 window
        ...
        # Slideshow images list (replace with your actual image paths)
    ...
    # Change image every 3 seconds with animation
        def change_slide(dt):
            next_slide = (self.current_slide + 1) % len(self.slideshow_images)
    
    # Cross-fade (both images animate simultaneously)
            Animation(opacity=1, duration=1.5).start(self.slideshow)  # Current fades out
            self.slideshow.source = self.slideshow_images[next_slide]  # Set next image
            Animation(opacity=1, duration=1.5).start(self.slideshow)  # Next fades in

            self.current_slide = next_slide

        # Start the slideshow and keep a reference to the clock event
        self.slideshow_event = Clock.schedule_interval(change_slide, 6)
    ...
    # Slide back in welcome screen
        ...
        # Slide back in welcome screen
        anim.start(self.main_layout)
    # Restart slideshow when returning to welcome screen
        self.start_slideshow()
        
        
    def show_role_intro(self, instance):
    # Slide out welcome screen
        # Cancel the slideshow animation when leaving welcome screen
    ...
    def start_registration(self, instance=None):
        ...
        # Create the title label with animation
        title = Label(
        ...
        anim = (Animation(font_size=dp(32), color=(1, 1, 0.5, 1), duration=1.5, t='out_quad') + 
                Animation(font_size=dp(28), color=(0.9, 0.9, 0.1, 1), duration=1.5, t='out_quad'))
        anim.repeat = True
        anim.start(title)
        ...
        for i in range(8):
            ...
            player_input = TextInput(
                ...
                foreground_color=(1, 1, 1, 1),  # Main text color
                hint_text_color=(0.9, 0.9, 1, 1),  # <-- Add this line for hint text color
                cursor_color=(1, 1, 1, 1),
                font_size=dp(20),
                hint_text=f"Player {i+1} Name (must include number)",
                padding=dp(10)
            )
    ...
    def validate_registration(self, instance):
        ...
        # 2. Check for at least 1 number in each name
        for name in self.player_names:
            if not any(char.isdigit() for char in name):
                self.show_error_popup("Each username must contain at least one number such as Alice8")
                return

        # 3. Check for EXACT duplicates (case-sensitive)
        if len(set(self.player_names)) != len(self.player_names):
            self.show_error_popup("All Player names must be completely unique")
            return
    ...
    def update_role_reveal(self):
        ...
        role_screen.role_text = 'Your Role is: ' + role_data['role'].upper()  # Set combined text
    ...
    def process_votes(self):
        ...
        if role == 'Citizen':
            message = "Masoom citizen mar gaya"  # Plain text without color tags
            image = 'assets/eliminated.png'
        elif role == 'Thief':
            message = "Ek chor pakda gaya"  # Plain text without color tags
            image = 'assets/thief_died.png'
        else:  # Angel
            message = "Angel bhi mar gaya"  # Plain text without color tags
            image = 'assets/angel_died.png'

        self.show_popup(
            "Voting Result",  # Plain text title
            message,
            self.show_thieves_phase,
            image
        )
    ...
    def show_thieves_phase(self):
        ...
        for player in targets:
            ...
            # Fixed callback binding - creates proper closure
            def make_callback(p):
                return lambda instance: self.set_thieves_target(p)

            card.bind(on_card_pressed=make_callback(player))
            grid.add_widget(card)
    ...
    # Create a debug popup to clearly show the selected target
        debug_popup = Popup(title='Thieves Target',
                           size_hint=(0.8, 0.5))

        content = BoxLayout(orientation='vertical')
        debug_label = Label(text=f"Thieves selected: {target}",
                           font_size='50sp',  # Very large font size
                           color=(1, 1, 1, 1),  # Red color for visibility
                           bold=True)
        ...
        print(f"Thieves selected: {target}")  # Also keep console log
    ...
    def show_angel_phase(self):
        ...
        instruction = Label(
            text=f"Angel {angel}, choose someone to save anyone including yourself:",
            font_size=dp(30),
            outline_color=(0,0,0,0),
            outline_width=2,
            bold=True,
        ...
        for player in targets:
            ...
            # Fixed callback binding - creates proper closure
            def make_callback(p):
                return lambda instance: self.set_angel_choice(p)
    ...
    def update_alive_players(self):
        """Sync both alive and eliminated player lists"""
    ...
    def set_angel_choice(self, target):
        # Show debug popup with big red bold text
        ...
        # Determine message and image based on scenario
    ...
    def show_round_results(self):
        self.setup_background("results")  # <-- Add this
        self.clear_widgets()
    ...
    def show_final_results(self, message):
        self.setup_background("gameover")  # <-- Add this
        self.clear_widgets()
    ...
    def show_popup(self, title, message, callback, image=None):
        ...
        # Remove color tags from title if present
        ...
        # Main message label with bold text
        message_label = Label(
            text=message,
            font_size=dp(26),  # Slightly larger font
            markup=True,
        ...
        popup = Popup(
            title=clean_title,  # Use cleaned title without color tags
            title_size=dp(24),
            title_align='center',
    ...
    def show_error_popup(self, message):
        ...
        popup = Popup(
            ...
            title_font='Roboto-Bold.ttf'  # This will make the title bold
        )
