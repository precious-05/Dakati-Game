...
class DakatiGame(BoxLayout):
    ...
    def update_role_reveal(self):
        ...
        if role_data['role'] == 'Thief':
            partner_label = Label(
                text=f"Your partner: {role_data['partner']}",
                font_size=dp(24),
                color=(0.9, 0.6, 0.1, 1)
            )
    ...
    def show_thieves_phase(self):
        ...
        # Instruction
        instruction = Label(
            text=f"Thieves {' & '.join(thieves)}, choose a target:",
            font_size=dp(20),
            color=(0.8, 0.8, 0.8, 1),
            size_hint=(1, 0.1)
        )
    ...
    def set_angel_choice(self, target):
        ...
        if self.angel_choice == self.thieves_target:
            message = "The angel saved the target!\nThe thieves' attack failed!"
        else:
            if self.players[self.thieves_target]['role'] in ['Citizen', 'Angel']:
                ...
                message = f"The angel failed to save {self.thieves_target}!"
            else:
