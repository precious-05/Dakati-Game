...
class DakatiGame(BoxLayout):
    ...
    def show_thieves_phase(self):
        ...
        for player in targets:
            ...
            card.alive = self.players[player]['alive']
            card.show_role = False
            card.bind(on_card_pressed=lambda instance: self.set_thieves_target(player))
            grid.add_widget(card)
    ...
    def show_angel_phase(self):
        ...
        for player in targets:
            ...
            card.alive = self.players[player]['alive']
            card.show_role = False
            card.bind(on_card_pressed=lambda instance: self.set_angel_choice(player))
            grid.add_widget(card)
    ...
    def set_angel_choice(self, target):
        ...
        if self.angel_choice == self.thieves_target:
            message = "[color=000000]Jo chor citizen ko marna chahte the wo bach gaya Angel ki wajah se[/color]"
            ...
            if self.players[self.thieves_target]['role'] in ['Citizen', 'Angel']:
                ...
                role = self.players[self.thieves_target]['role']
                if role == 'Citizen':
                    message = "[color=000000]Angel ne galat guess kia! Masoom citizen mar gaya![/color]"
                    image = 'assets/eliminated.png'
                else:  # Angel
                    message = "[color=000000]Angel khud ko nahi bacha paya! Angel mar gaya!\n[/color]"
                    image = 'assets/angel_died.png'
            else:
                message = "[color=000000]The thieves' target was a fellow thief!\nNo one was eliminated[/color]"
                image = 'assets/nothing.png'

        self.show_popup(
            "[color=000000]Angel's Decision[/color]",
            message,
            self.show_round_results,
            image
        )
