...
class DakatiGame(BoxLayout):
    ...
    # Cross-fade (both images animate simultaneously)
        ...
        self.slideshow_event = Clock.schedule_interval(change_slide, 8)
    ...
    def process_votes(self):
        ...
        self.update_alive_players()  # Update both lists
        self.eliminated_in_voting = eliminated

        role = self.players[eliminated]['role']
        if role == 'Citizen':
            message = "[color=000000]Masoom citizen mar gaya[/color]"
            image = 'assets/eliminated.png'
        elif role == 'Thief':
            message = "[color=000000]Ek chor pakda gaya[/color]"
            image = 'assets/thief_died.png'
        else:  # Angel
            message = "[color=000000]Angel bhi mar gaya[/color]"
            image = 'assets/angel_died.png'

        self.show_popup(
            "[color=ffffff]Voting Result[/color]",
            message,
            self.show_thieves_phase,
            image
        )
    ...
    # Create a debug popup to clearly show the selected target
        debug_popup = Popup(title='DEBUG: Thieves Target',
                           size_hint=(0.8, 0.5))
    ...
    def set_angel_choice(self, target):
        print(f"Angel chose: {target}")  # Debug log
        self.angel_choice = target
        if self.angel_choice == self.thieves_target:
            message = "Angel saved the target!"
        else:
            self.players[self.thieves_target]['alive'] = False
            message = "Angel failed to save the target!"

        self.update_alive_players()
        self.show_popup("Angel's Decision", message, self.show_round_results)
    ...
    def show_popup(self, title, message, callback, image=None):
        ...
        popup = Popup(
            ...
            title_size=dp(20),
            background='assets/popup_bg.png'  # Dark background
        )
